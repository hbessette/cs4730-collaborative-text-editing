...
Operation Serializer::decode(const std::vector<uint8_t> &bytes) {
    ...
    if (static_cast<MsgType>(msgTypeByte) != MsgType::OPERATION) {
        throw std::runtime_error("Serializer::decode: expected OPERATION message type");
    }

    uint32_t payloadLen = readUint32(data, offset);
    if (bytes.size() - offset < payloadLen) {
        throw std::runtime_error("Serializer::decode: buffer too short for payload");
    }
    if (payloadLen < 18) {
...
std::vector<uint8_t> Serializer::encodeState(const CRDTEngine &engine) {
    ...
    for (const auto &node : engine.seq_) {
        if (node.id == SENTINEL_ID) continue;
        ++nodeCount;
    }
...
CRDTEngine Serializer::decodeState(const std::vector<uint8_t> &bytes) {
    if (bytes.size() < 5) {
        throw std::runtime_error("Serializer::decodeState: buffer too short for header");
    }
    ...
    if (static_cast<MsgType>(msgTypeByte) != MsgType::STATE) {
        throw std::runtime_error("Serializer::decodeState: expected STATE message type");
    }

    uint32_t payloadLen = readUint32(data, offset);
    if (bytes.size() - offset < payloadLen) {
        throw std::runtime_error("Serializer::decodeState: buffer too short for payload");
    }
    if (payloadLen < 12) {
        throw std::runtime_error("Serializer::decodeState: state payload too short");
    }
    ...
    if (payloadLen < expectedPayload) {
        throw std::runtime_error("Serializer::decodeState: payload too short for node count");
    }
